...
class ModuleSectionConfigService:
    ...
    async def get_configs_by_workspace(self, db: AsyncSession, workspace_id: int) -> List[ModuleSectionConfig]:
        """获取指定工作区的模块配置（已废弃，请使用get_workspace_module_configs）"""
        try:
            return await module_section_config_repository.get_configs_by_workspace(db, workspace_id)
        except Exception as e:
    ...
    async def get_workspace_module_configs(self, db: AsyncSession, workspace_id: int) -> List[ModuleConfigForWorkspaceResponse]:
        """获取指定工作区的模块配置"""
        return await workspace_module_config_service.get_workspace_module_configs(db, workspace_id)
    ...
    async def update_configs(
        ...
        """更新模块配置（已废弃，请使用update_configs_by_workspace）"""
        try:
    ...
    async def update_configs_by_workspace(
        self,
        db: AsyncSession,
        workspace_id: int,
        configs_to_update: List[ModuleSectionConfigUpdate]
    ) -> List[ModuleSectionConfig]:
        """更新指定工作区的模块配置（已废弃，请使用update_workspace_module_configs）"""
        try:
            config_ids = [c.id for c in configs_to_update]

            # 1. 从数据库中获取现有的对象，并验证它们属于指定工作区
            existing_configs_list = await module_section_config_repository.get_by_ids_and_workspace(db, config_ids, workspace_id)
            existing_configs_map = {c.id: c for c in existing_configs_list}
    ...
    async def update_workspace_module_configs(
        ...
        """更新指定工作区的模块配置"""
        return await workspace_module_config_service.update_workspace_module_configs(db, workspace_id, configs_to_update)
